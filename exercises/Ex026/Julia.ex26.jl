...
for i in 2:999
    ...
    remainders_found = [false for x in 1:(i-1)]
    pattern_length = 0
    pattern_found = false

    while n != 0
        n *= 10                     # go to the next significant figure
        wholes = floor(Int, n / i)  # get the number of times n is evenly divisible by i
        n = n % i
        push!(result, wholes)
        if n == 0
            break
        ...
        if remainders_found[n]
            idx1 = findfirst(v->v==wholes, result)
            idx2 = length(result)
            pattern_length = idx2 - idx1
            if pattern_length == 0
                if length(findall(v->v==true, remainders_found)) == 1
                    pattern_length = 1
                end
            end
            if pattern_length > longest_pattern
                global longest_pattern, longest_divisor
    ...
    #println(i, "->  0.", join(result), " -> ", pattern_length)
end

println(longest_divisor)  # 983
